...
class CloudWatchMetrics:
    ...
    def __init__(self) -> None:
        ...
        self.namespace = (
            METRICS_NAMESPACE_DEV if settings.env == "dev" else METRICS_NAMESPACE_PROD
        )
    ...
    async def publish_prediction_metrics(
        ...
        try:
            ...
            high_conf_count = (
                sum(1 for c in confidence_values if c > 0.9) if confidence_values else 0
            )
            high_conf_percentage = (
                (high_conf_count / total_predictions * 100)
            ...
            logger.error(
                "Failed to publish metrics", request_id=request_id, error=str(e)
            )
    ...
    def _add_business_metrics(
        ...
        high_confidence_percentage = (
            (high_confidence_count / total_predictions * 100)
            if total_predictions > 0
            else 0
        )
